...
import { getCoursesForUser, addCourse, updateCourse, deleteCourse as deleteCourseFromDb, getFriends } from '@/lib/firestore';
...
export default function LearnPage() {
  ...
  const handleGenerateCourse = async (input: GenerateFullCourseInput) => {
    ...
    try {
      ...
      const steps: Step[] = result.course.map(step => {
        ...
        if (step.funFact) newStep.funFact = step.funFact;
        if (step.externalLinks) newStep.externalLinks = step.externalLinks;
      ...
      const newCourseData = {
        ...
        userId: user.uid, userName: user.displayName || user.email?.split('@')[0] || "Anonymous", isPublic: false,
      };
      ...
      console.error(error);
      toast({ variant: "destructive", title: "Error Generating Course", description: error instanceof Error ? error.message : "An unknown error occurred." });
  ...
  const mainContent = activeCourse ? (
    ...
    <div className="h-full flex items-center justify-center p-4 md:p-8">
      <TopicSelection onGenerateCourse={handleGenerateCourse} generationState={generationState} />
    </div>
  );
